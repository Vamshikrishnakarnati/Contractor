				...
				<div class="card card-statistics">
					<div class="card-header">
						<div class="card-heading text-right">
							<a href="<?=site_url();?>ctt-admin/department-listing" class="btn btn-primary btn-sm"><i class="ti ti-arrow-left"></i> <?=$BTN_Back;?></a>
						</div>
					</div>
					<div class="card-body">
						<form id="userForm" method="post" class="form-horizontal" enctype="multipart/form-data" autocomplete="off">
							<div class="row">
								<div class="form-group col-md-6">
									<label class="control-label" for="department_name"><?=$MD_Department_Name;?> <span class="text-danger">*</span></label>
									<div class="mb-2">
								...
								<div class="form-group col-md-6">
									<label class="control-label" for="department_code"><?=$MD_Department_Code;?> <span class="text-danger">*</span></label>
									<div class="mb-2">
										<input type="text" class="form-control" id="department_code" name="department_code" placeholder="Enter Department Code" maxlength="10" value="<?php if($urisegment) { if(isset($_POST['department_code'])) { echo $_POST['department_code']; } else { echo $this->admin->HtmlStripSlash($EditData->department_code); } }  else { if(isset($_POST['department_code'])) { echo $_POST['department_code']; } } ?>"/>
									</div>
								</div>
								<div class="form-group col-md-12">
									<?php if($urisegment) { ?>
									<button type="submit" class="btn btn-primary" name="btn_update" value="updatedepartment"><?=$BTN_Update;?></button>
									<?php } else { ?>
									<button type="submit" class="btn btn-primary" name="btn_submit" value="adddepartment"><?=$BTN_Submit;?></button>
									<?php } ?>
									<a href="<?=site_url();?>ctt-admin/department-listing" class="btn btn-danger"><?=$BTN_Cancel;?></a>
