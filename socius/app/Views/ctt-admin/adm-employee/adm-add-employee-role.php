				...
				<div class="card card-statistics">
					<div class="card-body">
							...
							<div class="position-sticky pt-3" style="top:60px;z-index:99;background:#ffffff;">
								<div class="form-row">
									...
									<div class="col-md-3 align-self-end mb-3">
										<button type="submit" value="submit" name="btn_submit" id="btn_submit" class="btn btn-primary btn-sm"><?=$BTN_Search?></button>
									</div>
									<div class="col-md-3 align-self-end text-right mb-3">
										<button type="button" class="btn btn-primary btn-sm"><?=$MER_All_Save;?></button>
						...
						<div class="mt-4">
							<div class="row">
								<div class="col-md-12">
									<div class="table-responsive" style="display: <?php if(!empty($QsEmployee)) { echo "block"; } else { echo "none"; }?>;">
										<form role="form" name="rolelistingForm" id="rolelistingForm" method="post">
											<table id="datatable" class="table table-hover table-bordered w-100">
												...
												<tbody>
													<tr>
														<input type="text" name="row_id" id="row_id" value="<?=$RsEmployee->id;?>">
														...
														<td>
															<select class="custom-select" name="level_<?=$RsEmployee->id;?>" id="level_<?=$RsEmployee->id;?>" onchange="get_department(<?=$RsEmployee->id;?>);">
																...
																<option value="<?=$RsAccessLevel->id;?>" <?php if($EditData->organisation_id == $RsAccessLevel->id) {?>selected<?php } ?> ><?=$this->admin->HtmlStripSlash($RsAccessLevel->name);?></option>
																<?php } ?>
															</select>
														</td>
														<td>
															<select class="custom-select" name="level2_<?=$RsEmployee->id;?>" id="level2_<?=$RsEmployee->id;?>">
																...
																<option value="<?=$RsAccessLevel2->id;?>" <?php if($EditData->organisation_id == $RsAccessLevel2->id) {?>selected<?php } ?> ><?=$this->admin->HtmlStripSlash($RsAccessLevel2->name);?></option>
																<?php } ?>
															</select>
														</td>
														<td>
												...
												<?php } } //print_r($ids);?>
												<input type="text" name="row_ids" id="row_ids" value="<?=$ids;?>">
											</table>	
										</form>	
									</div>
								</div>
