			...
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
                            ...
                            <div class="row">
								<div class="form-group col-md-6">
									<div class="form-inline">
										<div class="mb-2 mr-2">
											<input type="text" autocomplete="off" onkeyup="searchdata();" class="form-control" id="employee_name" name="employee_name" placeholder="Enter Employee Name" maxlength="80" value=""/>
										</div>
										<div class="mb-2">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-employee" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?> <?=$ME_Employee;?></a>
										<a href="<?=site_url();?>ctt-admin/employeerole-listing" class="btn btn-primary"> <?=$ManageEmployeeRole;?></a>
									</div>
                                </div>
                            </div>
                        </form>
						...
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsEmployee)) { echo "block"; } else { echo "none"; }?>;">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$ME_Employee_Name;?></th>
										<th><?=$ME_Employee_Email;?></th>
								...
								<tbody>
									...
									if(!empty($QsEmployee))//Check Count
									{
										...
										{ 
										$i++;
										$Status = $RsEmployee->is_active;
										if($Status == 1) 
										{ 
											$StatusFun = "inactive-employee"; 
									...
									<tr class="text-center">
										...
										<td><?=$this->admin->HtmlStripSlash($RsEmployee->employee_email);?></td>
										<td>
											<div class="btn-group btn-group-sm">
												<!--<button type="button" title="<?=$title;?>" class="btn btn-sm btn-<?=$Class;?>" data-toggle="modal" data-userid="<?=$RsEmployee->id;?>" data-title="<?=$title;?>" data-statusfun="<?=$StatusFun;?>" data-target="#modal-active-confirm"><span class="<?=$icon;?>"></span></button>&nbsp;&nbsp;-->
												<a href="<?=site_url();?>ctt-admin/edit-employee/<?=$RsEmployee->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
												<button type="button" class="btn btn-sm btn-danger"  data-toggle="modal"  data-deluserid="<?=$RsEmployee->id;?>" data-target="#modal-delete-confirm" title="Delete" style="background-color: #f71a1a"><span class="fa fa-trash"></span></button>
											</div>
										</td>
									</tr>
									<?php } } ?>
						...
						<?php if(empty($QsEmployee) && !empty($_POST)) { ?>
...
$(function() {
    $("#employee_name").autocomplete({
		source: function(request, response) {
			$.ajax({
				url: site_url+'ctt-admin/search-employee',
				dataType: "json",
				data: {
