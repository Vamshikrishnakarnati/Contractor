			...
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
                            ...
                            <div class="row">
								<div class="form-group col-md-4">
									<div class="mb-2 selects-contant">
										<select class="js-basic-single form-control cat" name="sub_dept_id" id="sub_dept_id" <?php if(isset($_POST['sub_dept_id'])) { ?>selected <?php } ?>>
											<option value="">Select Evaluation</option>
											<?php 
											$QsSubDept = $this->admin->retrive_all_cond_data('*',tbl_sub_department, 'is_active=1 AND is_delete = 0','id','ASC');
											if(!empty($QsSubDept))
											{
												foreach($QsSubDept as $RsSubDept) {
											?>
											<option value="<?=$RsSubDept->id;?>" <?php if(!empty($_POST['sub_dept_id']) && $_POST['sub_dept_id'] == $RsSubDept->id) {?>selected<?php } ?> ><?=$this->admin->HtmlStripSlash($RsSubDept->question_set_name);?></option>
											<?php } } ?>
										</select>
									</div>
								</div>
                                ...
                                <div class="col-md-8">
                                   <!-- <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-evaluation-questions" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?></a>
									</div>-->
                                </div>
                            </div>
                        </form>
					
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsEvaluationQuestions)) { echo "block"; } else { echo "none"; }?>;">
						<table id="datatable" class="table table-hover table-bordered w-100">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th width="50px;"><?=$MEQ_EvaluationQuestion;?></th>
									...
									$QsEvaluationPoints = $this->admin->retrive_all_cond_data('*',tbl_evaluation_points,'is_delete="0" AND evaluation_question_id = "'.$RsEvaluationQuestions-> id.'"', 'id','ASC'); 
										...
										{	
											...
											?>
											  ...
											  <li><?php echo $RsEvaluationPoints->point_name; ?> - <?php echo $RsEvaluationPoints->points; ?> Points</li>
										<?php  	 } } ?>	
										</ol>
										</td>
						...
						<?php if(empty($QsEvaluationQuestions) && !empty($_POST)) { ?>
							<div class="datatable-wrapper">
							<table id="datatable" class="table table-hover table-bordered w-100">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th width="10px;"><?=$SL;?></th>
										<th width="150px;"><?=$MEQ_EvaluationQuestion;?></th>
...
$(document).ready(function () {
  ...
  ignore: [],
	...
	messages: {
	  sub_dept_id: {
		required: "Please select evaluation",
	  },
	},
	errorElement: 'span',
...
function get_evaluation_values(sub_dept_id)
{
	var projectPath = $("#projectPath").val();
	$.ajax(
	 { 
		url: site_url+'ctt-admin/search-evaluation-questions',
		  type: "POST",
		  cache: false,
...
// source: site_url+'ctt-admin/search-industry/c',
$(function() {
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-evaluation-questions/'+deluserid;
  var modal = $(this)
