		...
		<div class="row">
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
						<form role="form" name="userlistingForm" id="userlistingForm" method="post">
                            <div class="row">
								<div class="form-group col-md-4">
									<div class="mb-2">
										<input type="text" autocomplete="off" onkeyup="ss();" class="form-control" id="admin_name" name="admin_name" placeholder="Enter Admin Name" maxlength="80" value="<?php if($urisegment) { if(isset($_POST['admin_name'])) { echo $_POST['admin_name']; } else { echo $this->admin->HtmlStripSlash($EditData->admin_name); } }  else { if(isset($_POST['admin_name'])) { echo $_POST['admin_name']; } } ?>"/>
									</div>
								</div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-key-manager" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?></a>
									</div>
                                </div>
                            </div>
                        </form>
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsKeyManager)) { echo "block"; } else { echo "none"; }?>;">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$MKM_KeyManagerName;?></th>
										<th><?=$MKM_KeyManagerId;?></th>
										<th><?=$MKM_KeyManagerEmail;?></th>
										<th><?=$MKM_KeyManagerPhone;?></th>
										<th data-sortable="false" class="notexport"><?=$Action;?></th>
									</tr>
								</thead>
								<tbody >
									<?php 
									if(!empty($QsKeyManager))//Check Count
									{
										$i=0;
										$usertype = "";
									...
									<tr class="text-center">
										<td><?=$i;?></td>
										<td><?=$this->admin->HtmlStripSlash($RsKeyManager->admin_name);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsKeyManager->admin_id);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsKeyManager->admin_email);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsKeyManager->admin_phone);?></td>
										<td>
											<div class="btn-group btn-group-sm">
												<!--<button type="button" title="<?=$title;?>" class="btn btn-sm btn-<?=$Class;?>" data-toggle="modal" data-userid="<?=$RsKeyManager->id;?>" data-title="<?=$title;?>" data-statusfun="<?=$StatusFun;?>" data-target="#modal-active-confirm"><span class="<?=$icon;?>"></span></button>&nbsp;&nbsp;-->
												<a href="<?=site_url();?>ctt-admin/edit-key-manager/<?=$RsKeyManager->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
												<button type="button" class="btn btn-sm btn-danger"  data-toggle="modal"  data-deluserid="<?=$RsKeyManager->id;?>" data-target="#modal-delete-confirm" title="Delete" style="background-color: #f71a1a"><span class="fa fa-trash"></span></button>
											</div>
										</td>
									</tr>
									<?php } } ?>
						...
						<?php if(empty($QsKeyManager) && !empty($_POST)) { ?>
...
$(document).ready(function () {
  ...
  ignore: [],
	...
	messages: {
	  admin_name: {
		required: "Please enter key manager name",
	  },
	},
	errorElement: 'span',
...
$(function() {
    $("#admin_name").autocomplete({
		source: function(request, response) {
			$.ajax({
				url: site_url+'ctt-admin/search-key-manager',
				dataType: "json",
				data: {
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-key-manager/'+deluserid;
  var modal = $(this)
