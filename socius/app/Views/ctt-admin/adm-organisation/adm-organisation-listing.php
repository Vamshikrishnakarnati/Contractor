			...
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-organisation" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?></a>
									</div>
                                </div>
                            </div>
                        </form>
						
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsOrganisation)) { echo "block"; } else { echo "none"; }?>;">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$MO_Company_Name;?></th>
										<th><?=$Status;?></th>
								...
								<tbody >
									<?php 
									if(!empty($QsOrganisation))//Check Count
									{
										$i=0;
										foreach($QsOrganisation as $RsOrganisation)
										{ 
										$i++;
										$Status = $RsOrganisation->is_active;
										if($Status == 1) 
										{ 
											$StatusFun = "inactive-organisation"; 
									...
									<tr class="text-center">
										<td><?=$i;?></td>
										<td><?=$this->admin->HtmlStripSlash($RsOrganisation->company_name);?></td>
										<td class="text-<?=$TextClass;?>"><?=$StatusTitle?></td>
										<td>
											<div class="btn-group btn-group-sm">
												<!--<button type="button" title="<?=$title;?>" class="btn btn-sm btn-<?=$Class;?>" data-toggle="modal" data-userid="<?=$RsOrganisation->id;?>" data-title="<?=$title;?>" data-statusfun="<?=$StatusFun;?>" data-target="#modal-active-confirm"><span class="<?=$icon;?>"></span></button>&nbsp;&nbsp;-->
												<a href="<?=site_url();?>ctt-admin/edit-organisation/<?=$RsOrganisation->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
												<button type="button" class="btn btn-sm btn-danger"  data-toggle="modal"  data-deluserid="<?=$RsOrganisation->id;?>" data-target="#modal-delete-confirm" title="Delete" style="background-color: #f71a1a"><span class="fa fa-trash"></span></button>
											</div>
										</td>
									</tr>
									<?php } } ?>
						...
						<?php if(empty($QsOrganisation) && !empty($_POST)) { ?>
...
$(function() {
    $("#company_name").autocomplete({
		source: function(request, response) {
			$.ajax({
				url: site_url+'ctt-admin/search-organisation',
				dataType: "json",
				data: {
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-organisation/'+deluserid;
  var modal = $(this)
