					...
					<div class="ml-auto d-flex align-items-center">
						<nav>
							<ol class="breadcrumb p-0 m-b-0">
								...
								<li class="breadcrumb-item active text-primary" aria-current="page"><?php if($urisegment) { echo $BTN_Edit; } else { echo $BTN_Add; } ?> <?=$ST_Parameter;?></li>
							</ol>
						</nav>
					</div>
				</div>
				...
				<div class="card card-statistics">
					<div class="card-header">
						<div class="card-heading text-right">
							<a href="<?=site_url();?>pk-admin/parameter-listing" class="btn btn-primary btn-sm"><i class="ti ti-arrow-left"></i> <?=$BTN_Back;?></a>
						</div>
					</div>
					<div class="card-body">
						<form id="parameterForm" method="post" class="form-horizontal" enctype="multipart/form-data" autocomplete="off">
							<div class="row">
								<div class="form-group col-md-6">
									<label class="control-label" for="user_name"><?=$ST_Parameter_Name;?> <span class="text-danger">*</span></label>
									<div class="mb-2">
										<input type="text" class="form-control" id="parameter_name" name="parameter_name" placeholder="Enter Parameter Name" maxlength="80" value="<?php if($urisegment) { if(isset($_POST['parameter_name'])) { echo $_POST['parameter_name']; } else { echo $this->admin->HtmlStripSlash($EditData->parameter_name); } }  else { if(isset($_POST['parameter_name'])) { echo $_POST['parameter_name']; } } ?>"/>
									</div>
								</div>
							...
							<div class="modal fade" id="modal-delete-inactive-confirm" tabindex="-1" role="dialog" aria-hidden="true">
								...
								<div class="form-group col-md-12">
									<?php if($urisegment) { ?>
									<button type="submit" class="btn btn-primary" name="btn_update" value="updateuser"><?=$BTN_Update;?></button>
									<?php } else { ?>
									<button type="submit" class="btn btn-primary" name="btn_submit" value="addparameter"><?=$BTN_Submit;?></button>
									<?php } ?>
									<a href="<?=site_url();?>pk-admin/parameter-listing" class="btn btn-danger"><?=$BTN_Cancel;?></a>
