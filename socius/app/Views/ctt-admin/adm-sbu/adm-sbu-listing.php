		...
		<div class="row">
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
						<form role="form" name="discountpointlistingForm" id="discountpointlistingForm" method="post">
                            <div class="row">
								<div class="form-group col-md-4">
									<div class="mb-2">
										<input type="text" autocomplete="off" onkeyup="searchlisting();" class="form-control" id="sbu_name" name="sbu_name" placeholder="Enter Sbu Name" maxlength="80" value="<?php if($urisegment) { if(isset($_POST['sbu_name'])) { echo $_POST['sbu_name']; } else { echo $this->admin->HtmlStripSlash($EditData->sbu_name); } }  else { if(isset($_POST['sbu_name'])) { echo $_POST['sbu_name']; } } ?>"/>
									</div>
								</div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-sbu" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?></a>
									</div>
                                </div>
                            </div>
                        </form>
						
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsSbu)) { echo "block"; } else { echo "none"; }?>;">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$MSBU_OrganisationName;?></th>
										<th><?=$MSBU_SBUName;?></th>
										<!--<th><?=$Status;?></th>-->
								...
								<tbody >
									<?php 
									if(!empty($QsSbu))//Check Count
									{
										$i=0;
										foreach($QsSbu as $RsSbu)
										{ 
										$i++;
										$OrganisationName = $this->admin->getDataById(tbl_organisation, 'id', $RsSbu->company_id);
									?>
									<tr class="text-center">
										<td><?=$i;?></td>
										<td><?=$this->admin->HtmlStripSlash($OrganisationName->company_name);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsSbu->sbu_name);?></td>
										<!--<td class="text-<?=$TextClass;?>"><?=$StatusTitle?></td>-->
										<td>
											<div class="btn-group btn-group-sm">
												<!--<button type="button" title="<?=$title;?>" class="btn btn-sm btn-<?=$Class;?>" data-toggle="modal" data-userid="<?=$RsSbu->id;?>" data-title="<?=$title;?>" data-statusfun="<?=$StatusFun;?>" data-target="#modal-active-confirm"><span class="<?=$icon;?>"></span></button>&nbsp;&nbsp;-->
												<a href="<?=site_url();?>ctt-admin/edit-sbu/<?=$RsSbu->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
												<button type="button" class="btn btn-sm btn-danger"  data-toggle="modal"  data-deluserid="<?=$RsSbu->id;?>" data-target="#modal-delete-confirm" title="Delete" style="background-color: #f71a1a"><span class="fa fa-trash"></span></button>
											</div>
										</td>
									</tr>
									<?php } } ?>
						...
						<?php if(empty($QsSbu) && !empty($_POST)) { ?>
...
$(document).ready(function () {
  $('#discountpointlistingForm').validate({
  ignore: [],
	rules: {
	  sbu_name: {
		required: true,
...
$(function() {
    $("#sbu_name").autocomplete({
		source: function(request, response) {
			$.ajax({
				url: site_url+'ctt-admin/search-sbu',
				dataType: "json",
				data: {
					sbu_name : $("#sbu_name").val()
				},
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-sbu/'+deluserid;
  var modal = $(this)
