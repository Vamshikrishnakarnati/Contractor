						...
						<div class="datatable-wrapper">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$ST_Parameter_Id;?></th>
										<th><?=$ST_Parameter_Value;?></th>
										<th data-sortable="false" class="notexport"><?=$Action;?></th>
									</tr>
								</thead>
							<tbody >
									<?php 
									if(count($QsSetting)>0)//Check Count
									{
										$i=0;
										foreach($QsSetting as $RsSetting)
										{ 
										$i++;
										$Status = $RsSetting->is_active;
										if($Status == 1) 
										{ 
											$StatusFun = "inactive-setting"; 
										...
										$RsParameter= $this->admin->getDataById(tbl_parameter,'id',$RsSetting->parameter_id);
									?>
									<tr class="text-center">
										<td><?=$i;?></td>
										<td><?=$this->admin->HtmlStripSlash($RsParameter->parameter_name);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsSetting->parameter_value);?></td>
										<td>
											<div class="btn-group btn-group-sm">
												
												<a href="<?=site_url();?>ctt-admin/edit-setting/<?=$RsSetting->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-setting/'+deluserid;
  var modal = $(this)
