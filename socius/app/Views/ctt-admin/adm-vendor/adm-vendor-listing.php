		...
		<div class="row">
			<div class="col-lg-12">
				<div class="card card-statistics">
					<div class="card-body">
						<form role="form" name="vendorlistingForm" id="vendorlistingForm" method="post">
                            <div class="row">
								<div class="form-group col-md-4">
									<div class="mb-2">
										<input type="text" autocomplete="off" onkeyup="ss();" class="form-control" id="company_name" name="company_name" placeholder="Enter Company Name" maxlength="150" value="<?php if($urisegment) { if(isset($_POST['company_name'])) { echo $_POST['company_name']; } else { echo $this->admin->HtmlStripSlash($EditData->company_name); } }  else { if(isset($_POST['company_name'])) { echo $_POST['company_name']; } } ?>"/>
									</div>
								</div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    <div class="mb-3 text-right">
										<a href="<?=site_url();?>ctt-admin/add-vendor" class="btn btn-primary"><i class="ti ti-plus"></i> <?=$BTN_Add;?></a>
									</div>
                                </div>
                            </div>
                        </form>
						<div class="datatable-wrapper" style="display: <?php if(!empty($QsVendor)) { echo "block"; } else { echo "none"; }?>;">
							<table id="datatable" class="table table-hover table-bordered w-100" style="margin-top: 0px !important;">
								<thead class="bg-primary text-white text-center">
									<tr>
										<th><?=$SL;?></th>
										<th><?=$MV_CompanyName;?></th>
										<th><?=$MV_CompanyLogo;?></th>
										<th><?=$MV_PhoneLandline;?></th>
										<th><?=$MV_Email;?></th>
										<th><?=$MV_AddressCorporate;?></th>
										<th><?=$Status;?></th>
								...
								<tbody >
									<?php 
									if(!empty($QsVendor))//Check Count
									{
										$i=0;
										foreach($QsVendor as $RsVendor)
										{ 
										$i++;
										$Status = $RsVendor->is_active;
										if($Status == 1) 
										{ 
											$StatusFun = "inactive-vendor"; 
									...
									<tr class="text-center">
										<td><?=$i;?></td>
										<td><?=$this->admin->HtmlStripSlash($RsVendor->company_name);?></td>
										<td>
											<div class="bg-img magnific-wrapper gallery mx-auto">
											<?php if($RsVendor->company_logo) { ?>	
												<a href="<?=site_url();?><?=$MV_CompanyLogoViewPath.$RsVendor->company_logo;?>" class="view1"><img class="img-fluid rounded" src="<?=site_url();?><?=$MV_CompanyLogoViewPath.$RsVendor->company_logo;?>" alt=""></a>
											<?php } else { ?>
												<img class="img-fluid rounded" src="<?=site_url();?><?=$NoImgPath?>" alt="">
											<?php } ?> 	
											</div>
										</td>
										<td><?=$this->admin->HtmlStripSlash($RsVendor->phone_landline);?></td>
										<td><?=$this->admin->HtmlStripSlash($RsVendor->email);?></td>
										<td>
										<?php if(strlen($RsVendor->address_corporate) > 50) { echo substr(($this->admin->HtmlStripSlash($RsVendor->address_corporate)),0,50);?><a style="text-decoration: none;color:#006df0;" href="" data-toggle="modal" class="btn-link" data-target="#blogModal<?=$RsVendor->id; ?>"><?=$BTN_ReadMore;?></a><?php } else { echo ($this->admin->HtmlStripSlash($RsVendor->address_corporate));} ?>
										</td>
											<div class="modal fade" id="blogModal<?=$RsVendor->id; ?>" tabindex="-1" role="dialog" aria-hidden="true">
												<div class="modal-dialog modal-dialog-centered" role="document">
													<div class="modal-content">
														<div class="modal-header">
															<h5 class="modal-title" id="verticalCenterTitle"><?=$MV_AddressCorporate; ?></h5>
														...
														<div class="modal-body">
															<p>
																<?=$this->admin->HtmlStripSlash($RsVendor->address_corporate); ?>
															</p>
														</div>
													</div>
												</div>
										...
										<td>
											<div class="btn-group btn-group-sm">
												<!--<button type="button" title="<?=$title;?>" class="btn btn-sm btn-<?=$Class;?>" data-toggle="modal" data-userid="<?=$RsVendor->id;?>" data-title="<?=$title;?>" data-statusfun="<?=$StatusFun;?>" data-target="#modal-active-confirm"><span class="<?=$icon;?>"></span></button>&nbsp;&nbsp;-->
												<a href="<?=site_url();?>ctt-admin/view-vendor/<?=$RsVendor->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-info"></i></a>&nbsp;&nbsp;
												<a href="<?=site_url();?>ctt-admin/edit-vendor/<?=$RsVendor->id;?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;
												<button type="button" class="btn btn-sm btn-danger"  data-toggle="modal"  data-deluserid="<?=$RsVendor->id;?>" data-target="#modal-delete-confirm" title="Delete" style="background-color: #f71a1a"><span class="fa fa-trash"></span></button>
											</div>
										</td>
									</tr>
									<?php } }  ?>
						...
						<?php if(empty($QsVendor) && !empty($_POST)) { ?>
							...
							<table id="datatable" class="table table-hover table-bordered w-100">
								...
								<tbody >
										...
										<tr><td colspan="8" style="text-align:center;color:red;"><?php echo $NoRecordsFound;?></td></tr>
									</table>
							</div>
						<?php } ?>
					</div>
...
// source: site_url+'ctt-admin/search-vendor/c',
$(function() {
    $("#company_name").autocomplete({
		source: function(request, response) {
			$.ajax({
				url: site_url+'ctt-admin/search-vendor',
				dataType: "json",
				data: {
...
$('#modal-delete-confirm').on('show.bs.modal', function (event) {
  ...
  var site_url = $('#projectPath').val();
  var StatusUrl = site_url+'ctt-admin/delete-vendor/'+deluserid;
  var modal = $(this)
